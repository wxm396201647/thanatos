package com.lib.network.config;

import android.app.Application;

import com.lib.network.interceptor.CustomSignInterceptor;
...
public class HttpRequest {
    ...
    public HttpRequest(Application application) {
        EasyHttp.init(application);//默认初始化
        initHttpConfig();
    }

    public static HttpRequest instance(Application application) {
        if (null == httpRequest) {
...
//                .addInterceptor(new GzipRequestInterceptor())//开启post数据进行gzip后发送给服务器
    ...
    public static void setBaseUrl(String baseUrl) {
        //可以全局统一设置全局URL
        EasyHttp.getInstance().setBaseUrl(baseUrl);//设置全局URL  url只能是域名 或者域名+端口号
    }
}
